...
main() {
  ...
  # The engine is detected fresh on every run (no persisted config); this is decided
  # before the incremental guard, so a missing engine signal doesn't get recorded in
  ...
  if [ -z "$engine" ]; then
    ...
    log_engine_guidance >>"$data_dir/logs/observer.log"
    return 0
  fi
