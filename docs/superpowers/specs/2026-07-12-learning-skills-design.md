...
| instincts の git 管理 | commit 対象外（`.learning/.gitignore` で除外。ローカル専用のランタイムデータ） |
| ランタイムデータの配置 | プロジェクト直下 `.learning/`（headless の claude は `.claude/` 配下への書き込みが保護により拒否されるため。実機検証で確認） |
...
```
<project>/.claude/skills/learning/   # コード（commit 対象）
...
<project>/.learning/                 # ランタイムデータ（初回実行時に自動作成、全体が commit 対象外）
├── instincts/                # 蓄積データ（1 Instinct = 1 Markdown）
├── logs/
│   └── observer.log          # 実行ログ
├── .lock                     # 多重起動防止ロック
└── .gitignore                # 「*」1行。observe.sh が自動生成し .learning 全体を除外
```

- commit 対象は SKILL.md・scripts/・prompts/ のみ。
- ランタイムデータを `.claude/` 配下に置かないのは、headless（`claude -p`）実行では `.claude/` 配下への Write/Edit が保護により拒否されるため（allowedTools 許可や acceptEdits でも不可。実機検証で確認）。
...
- `trap` でロック解放を保証。
- observer に許可するツールは `Read,Glob,Grep` に加え `Write`/`Edit` を `.learning/instincts/**` に限定する（プロンプトインジェクション対策: transcript 由来の指示が instincts 外のファイルを書き換えられないようにする）。
...
- `session-end.sh` はいかなる失敗でも exit 0。エラー詳細は `.learning/logs/observer.log` に追記するのみ。
