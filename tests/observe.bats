...
STUB
  ...
  # config は廃止済み。デフォルトでは CLAUDECODE=1 を立てて claude を検出させ、
  # 他エンジンのテストでのみ unset して PATH 上のスタブに切り替える
  export CLAUDECODE=1
...
@test "エンジンを検出できなければ実行せずログを出してロックを削除する" {
  unset CLAUDECODE
  run_observe
  [ "$status" -eq 0 ]
  [ ! -f "$TMP/claude-args.txt" ]
...
@test "エンジンを検出できなければ instincts ディレクトリや .gitignore を作らない" {
  unset CLAUDECODE
  run_observe
  [ "$status" -eq 0 ]
  [ ! -d "$DATA/instincts" ]
...
@test "codex が PATH にあれば codex exec が sandbox 付き・model なしで起動される" {
  unset CLAUDECODE
...
@test "engine=codex: LEARNING_SKILLS_MODEL があれば --model が付く" {
  unset CLAUDECODE
  make_engine_stub codex
  export LEARNING_SKILLS_MODEL=gpt-5.4-mini
  run_observe
  [ "$(arg_in_engine --model)" = "gpt-5.4-mini" ]
}

@test "copilot が PATH にあれば copilot -p が --no-ask-user と既定モデル付きで起動される" {
  unset CLAUDECODE
...
@test "CLAUDECODE=1 だが claude コマンドが無ければ起動せずロックを削除しエラーをログに出す" {
  run env CLAUDECODE=1 PATH=/usr/bin:/bin "$BIN/observe.sh" "$TMP/transcript.jsonl" "$TMP/project"
  [ "$status" -eq 0 ]
  [ ! -f "$TMP/claude-args.txt" ]
  [ ! -f "$DATA/.lock" ]
  [[ "$output" == *"required command not found: claude"* ]]
}
